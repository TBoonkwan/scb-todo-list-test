...
import 'package:scb_test/shared/passcode/heading_title.dart';
import 'package:scb_test/shared/passcode/input_state.dart';
import 'package:scb_test/shared/passcode/layout/key_pad.dart';
import 'package:scb_test/shared/passcode/layout/secrets.dart';
...
class ScreenLock extends StatefulWidget {
  ...
  /// Listener the text changed
  final void Function(String matchedText)? didTextChanged;

  final void Function()? confirmingPasscode;
...
class _ScreenLockState extends State<ScreenLock> {
  ...
  void inputListener() {
    inputState.currentInput.listen((text) {
      widget.didTextChanged?.call(text);
  ...
  void confirmInputListener() {
    inputState.currentInput.listen((text) {
      ...
      if (!firstInputCompleted) {
        ...
        widget.confirmingPasscode?.call();
      } else {
      ...
      if (success) {
        if (widget.confirmation) {
          if (widget.didConfirmed != null) {
            widget.didConfirmed?.call(firstInput);
          }
        } else {
          unlocked();
        }
